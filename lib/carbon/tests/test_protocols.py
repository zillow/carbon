...
class TestCacheManagementHandler(TestCase):
  ...
  def test_cache_query_precheck_response(self):
    ...
    self.assertEquals({'exists': True, 'partial_exists': True, 'should_ignore': False}, self.response)
  ...
  def test_cache_query_precheck_with_timestamp_False_response(self):
    ...
    self.assertEquals({'exists': False, 'partial_exists': True, 'should_ignore': False}, self.response)

  def test_cache_query_precheck_with_timestamp_False_response_should_not_ignore(self):
    self.cache.store('carbon.foo', (600, 1.0))
    self.cache.store('carbon.foo', (605, 1.0))
    self.send_request('cache-query-precheck', metric='carbon.foo', timestamp=601)
    self.assertEquals({'exists': True, 'partial_exists': True, 'should_ignore': False}, self.response)

  def test_cache_query_precheck_with_timestamp_False_response_should_ignore(self):
    self.cache.store('carbon.foo', (600, 1.0))
    self.cache.store('carbon.foo', (603, 1.0))
    self.send_request('cache-query-precheck', metric='carbon.foo', timestamp=604)
    self.assertEquals({'exists': True, 'partial_exists': True, 'should_ignore': True}, self.response)
